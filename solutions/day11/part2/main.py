...
class Monkey:
    ...
    def inspectItems(self, divisor):
        ...
        for i, _item in enumerate(self.items):
            ...
            if passesTest and self.items[i] % divisor != 0:
                self.items[i] = self.items[i] % divisor
            targets.append(
    ...
    def decreaseWorry(self, item, divisor):
...
def solution(lines):
    ...
    divisor = math.lcm(*[m.testDenominator for m in monkeys])
    for round in range(0, 10000):
        for i, monkey in enumerate(monkeys):
            targetMoneys = monkey.inspectItems(divisor)
